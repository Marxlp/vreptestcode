...
if clientID!=-1:
    ...
    _,quadrotor_base = vrep.simxGetObjectHandle(clientID,"Quadricopter_base", vrep.simx_opmode_oneshot_wait)
    _,quadrotor_target = vrep.simxGetObjectHandle(clientID, "Quadricopter_target", vrep.simx_opmode_oneshot_wait)
    
    target_position = [[-0.14999999105930328+0.5*x, 0.19999998807907104+sin(x),0.5] for x in np.linspace(0,2*np.pi,100)]
    vrep.simxGetObjectOrientation(clientID, quadrotor_base, -1, vrep.simx_opmode_streaming)

    for i in range(100):
        vrep.simxSetObjectPosition(clientID, quadrotor_target, -1,target_position[i], vrep.simx_opmode_oneshot)
        vrep.simxSynchronousTrigger(clientID)
        vrep.simxGetPingTime(clientID)
        _,angles = vrep.simxGetObjectOrientation(clientID, quadrotor_base, -1, vrep.simx_opmode_buffer)
        if res == vrep.simx_return_ok:
            print(angles)
        else:
            print('no data')
